...
class MergePanel extends MyBorderPanel {
	...
	MergePanel(DirectoryTable dt) {
		...
		String[] cols = {"Key","분석파일 1", "분석파일 2"};//key run1 run2
		...
		tp = addBorderPanel("합병 총합");//Merged Counts
		...
		save.addActionListener(
			new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					new TableSaver(parent,tm,jt,"상태");//Stats
				}
			}
		);
		p.add(save);
		p = addPanel("합병 옵션 지정", BorderLayout.NORTH);//Specify Merge Options
		...
		b = new JButton("합병 요약");//Merge Summaries     
	...
	public enum FilterOptions {
		All_Items("모든 항목 보기" , true),//Show All Items
		Matches("일치 항목 표시", true),//Show Matches
		Mismatches("불일치 항목 표시",true),//Show Mismatches
		ColHasValue("현재 값",false),//Value Present
		ColEmpty("값 누락",false);//Value Missing
