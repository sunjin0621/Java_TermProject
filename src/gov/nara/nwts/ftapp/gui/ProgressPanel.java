...
class ProgressPanel extends MyPanel {
	...
	String[] cols = {"탭 이름","파일 분석 종류","시작지점","완료","항목 수","걸린 시간","결과 파일"};
	//"Name","File Test","Root","Complete","Num Items","Duration","Output File"
	...
	ProgressPanel(DirectoryTable dt) {
		...
		p.add(cancel);
		p = addBorderPanel("테스트 기록");//Test Log
		...
		jt.repaint();
		batchp = addBorderPanel("일괄 처리");//Batch Processing
		batchp.setVisible(false);
		JPanel pp1 = new JPanel();
		JPanel pp2 = new JPanel();
		batchp.add(pp1, BorderLayout.NORTH);
		batchp.add(pp2, BorderLayout.SOUTH);
		batch = new FileSelectChooser(parent.frame, "일괄처리할 파일 선택하기 {시작지점->저장파일","");//Choose Batch input file {rootdir->saveFile
		pp1.add(batch);
		doBatch = new JButton("일괄 처리하기");//Process Batch
		pp2.add(doBatch);
		...
		pause = new JToggleButton("중지하기");//Pause
		...
		pp2.add(pause);
		clearBatch = new JButton("모두 지우기");//Clear Batch
	...
	void logTest(String name,String action, File root, boolean status, int numitems, double duration, File file){
		...
		v.add(status);
		v.add(numitems);
		v.add(FTDriver.ndurf.format(duration)+" 초 걸림");//secs
		v.add(file);
		tm.addRow(v);
	}
